...
TABLE_NAME = "volume_GF_plan_category"
FILE_PATH = "volume_gf_plan.xlsx"  # Путь к файлу
...
melted_df = df.melt(id_vars=base_columns, var_name="category_plan", value_name="PlanGF")

# Удаляем строки, где значение 'Plan' пустое (NaN)
melted_df = melted_df.dropna(subset=["PlanGF"])
...
# Вставка данных
sql_insert = f"""
    INSERT INTO {TABLE_NAME} (DSM, TSM, ESR, category_plan, PlanGF)
    VALUES (%s, %s, %s, %s, %s)
"""
